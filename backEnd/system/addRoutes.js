...
function addRouteTruck(id, truckId, route) {
    let def = Q.defer();
    dbConnection.getDb().collection(id).findOneAndUpdate({_id: truckId},
...
function correctFuel(id, truckId, route) {        
    dbConnection.getDb().collection(id).findOneAndUpdate({_id: truckId},
    {$pull: {"Truck.fuel": {fuel_Id: ObjectId(route.fuel_Id)}}}, (err, res) => {
        if(err) {return err;}
    
        dbConnection.getDb().collection(id).findOneAndUpdate({_id: route.driverId},
...
function calcRoute(id, truckId, route) {
    dbConnection.getDb().collection(id).findOne({_id: truckId}, (err, res) => {
        if (err) {return err;}
        if (!res) {return err;}   
...
function calcFuelDetails(truckId, route, res) {
...
function addFuel(id, truckId, obj) {        
    dbConnection.getDb().collection(id).findOneAndUpdate({_id:truckId},
    {$push: {"Truck.fuel" :obj}}, {upsert: true}, (err, res) => {if(err) {return err;}});        
    
    dbConnection.getDb().collection(id).findOneAndUpdate({_id:obj.driver},
    {$push: {"Driver.routes" :obj}}, {upsert: true}, (err, res) => {if(err) {return err;}
   });
}
